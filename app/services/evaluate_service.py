...
def evaluate_answer(correct_answers: List[str], user_answer: str) -> Dict[str, Any]:
    ...
    [Instructions]
    1. Determine if the User Answer is semantically consistent with one of the Correct Answers.
    2. Allow minor errors (spelling, articles, missing details).
    3. You MUST respond in valid JSON only.
    4. The "feedback" must be in English.

    {{
      "is_correct": true/false,
      "feedback": "Explain why it is correct or incorrect."
    }}
    """
    ...
    try:
        ...
        if response.status_code != 200:
            print("Evaluation API Error:", response.text)
            return {"is_correct": False, "feedback": "Evaluation failed due to API error."}
        ...
        return {"is_correct": False, "feedback": "JSON parsing error in evaluation result."}

    except Exception as e:
        print("Evaluation Error:", e)
        return {"is_correct": False, "feedback": "Evaluation error occurred."}
